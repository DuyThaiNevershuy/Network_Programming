...
void *thread_func(void *param)
{
    ...
    while (1)
    {
        ...
        int client_socket = accept(server, (struct sockaddr *)&client_address, &client_address_len);
        if (client_socket < 0)
        {
...
int main(int argc, char *argv[])
{
    ...
    if (bind(server, (struct sockaddr *)&server_address, server_address_len) < 0)
    {
        perror("bind() failed");
        exit(EXIT_FAILURE);
    }
    ...
    {
        if (pthread_create(&thread_pool[i], NULL, thread_func, (void *)&server) != 0)
        {
